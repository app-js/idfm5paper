...
void setup()
{
    ...
    log_d("idfm5: begin");
    M5.begin();
    log_d("idfm5: TP.SetRotation");
    M5.TP.SetRotation(90);
    log_d("idfm5: EPD.SetRotation");
    ...
    log_d("idfm5: loadFont");
    canvas_result.loadFont(binaryttf, sizeof(binaryttf));
    log_d("err: %d",err);

    log_d("idfm5: createRender");
    err = canvas_result.createRender(36, 14);
    log_d("err: %d",err);
    
    log_d("idfm5: for");
    for(int i = DEFAULT_FONT_SIZE; i >= 8; i-= 8)
    {
        log_d("idfm5: for createRender %d, 14\n",i);
        canvas_result.createRender(i, 14);
        log_d("idfm5: for setTextSize");
        canvas_result.setTextSize(i);
        log_d("idfm5: for for  preRender 0..9");
        for (int i = 0; i < 10; i++)
        {
            canvas_result.preRender('0' + i);
        }
        log_d("idfm5: for preRender .");
    ...
    log_d("idfm5: creatKeys");
    creatKeys();
    log_d("idfm5: updateValue");
    updateValue();
}

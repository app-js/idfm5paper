...
void* TFT_eSprite::createSprite(int16_t w, int16_t h, uint8_t frames)
{
  ...
  // log_e("callocSprite w%d h%d f%d",w,h,frames);
  _img8   = (uint8_t*) callocSprite(w, h, frames);
  // if(_img8 == NULL)
  // {
  //   log_e("callocSprite returns NULL");
  // }
...
size_t TFT_eSprite::write(uint8_t utf8)
{
  ...
  if ((!textwrapY) && (this->cursor_y + _write_font_height >= (_textarea_y + _textarea_h)))
  {
    if(_write_exceed_area == false)
    {
      _last_write_len = _write_len - _write_unicode_len;
      _write_len = 0;
      _write_exceed_area = true;
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  if (utf8 == '\n') 
  {
    ...
    this->cursor_x  = _textarea_x;
  }
  else
  {
    if (textwrapX && (this->cursor_x + width * textsize > (_textarea_x + _textarea_w)))
    {
      this->cursor_y += height;
      this->cursor_x = _textarea_x;
    }
    if (textwrapY && (this->cursor_y >= (_textarea_y + _textarea_h)))
    {
      this->cursor_y = _textarea_y;
    }
  ...
  else
  {
    if(utf8 == '\n') {
      ...
      if((w > 0) && (h > 0)) { // Is there an associated bitmap?
        ...
        if(textwrapX && ((this->cursor_x + textsize * (xo + w)) > (_textarea_x + _textarea_w))) {
          // Drawing character would go off right edge; wrap to new line
          this->cursor_x  = _textarea_x;
          this->cursor_y += (int16_t)textsize * yadvance;
        }
        if (textwrapY && ((this->cursor_y + yadvance) >= (_textarea_y + _textarea_h))) this->cursor_y = _textarea_y;
        drawChar(this->cursor_x, this->cursor_y + yadvance, uniCode, textcolor, textbgcolor, textsize);
      }
...
void TFT_eSprite::drawFreetypeBitmap(int32_t cx, int32_t cy, uint16_t bw, uint16_t bh, uint8_t *bitmap)
{
  ...
  for (int y = 0; y < bh; y++)
  {
    for (int x = 0; x < bw; x++)
    {
      if (getColor) bg = getColor(cx + x, cy + y);
      if(pos & 0x1)
      {
        drawPixel(cx + x, cy + y, alphaBlend(alphamap[bitmap[pos >> 1] & 0x0F], fg, bg));
      }
      else
      {
        drawPixel(cx + x, cy + y, alphaBlend(alphamap[bitmap[pos >> 1] >> 4], fg, bg));
      }
      pos++;
    }
  }
